...
func main() {
	...
	simpleCondBys, err := json.Marshal(simpleCond)
	if err != nil {
		fmt.Println(err.Error())
		return
	}
	...
	//printf
	{
		arrayList1, err := json.MarshalIndent(simpleCond, "", " ")
		if err != nil {
			fmt.Println(err.Error())
			return
		}
	...
	payLoad, err := fabric.QueryChaincode(fabConfig.ChaincodeID, "User1", "query_simple", [][]byte{simpleCondBys})
	if err != nil {
		fmt.Println(err.Error())
		return
	}
